import { Questions } from '@/types/questions.types'

export const questions: Questions = {
	quality: [
		{
			question: 'Question Service Quality',
			answers: [
				{ name: 'Excellent', color: '#4CAF50' },
				{ name: 'Good', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Poor', color: '#F44336' }
			]
		}
	],
	cleanliness: [
		{
			question: 'Question Cleanliness',
			answers: [
				{ name: 'Excellent', color: '#4CAF50' },
				{ name: 'Good', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Poor', color: '#F44336' }
			]
		}
	],
	tenants: [
		{
			question: 'Question Shops and tenants 1',
			answers: [
				{ name: 'Yes', color: '#4CAF50' },
				{ name: 'Partially', color: '#FFEB3B' },
				{ name: 'No', color: '#F44336' }
			]
		},
		{
			question: 'Question Shops and tenants 2',
			answers: [
				{ name: 'Yes', color: '#4CAF50' },
				{ name: 'Partially', color: '#FFEB3B' },
				{ name: 'No', color: '#F44336' }
			]
		}
	],
	comfort: [
		{
			question: 'Question Comfort (smell)',
			answers: [
				{ name: 'Excellent', color: '#4CAF50' },
				{ name: 'Good', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Poor', color: '#F44336' }
			]
		}
	],
	security: [
		{
			question: 'Question Security',
			answers: [
				{ name: 'Very satisfied', color: '#4CAF50' },
				{ name: 'Satisfied', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Not satisfied', color: '#F44336' }
			]
		}
	],
	restaurants: [
		{
			question: 'Question Restaurants 1',
			answers: [
				{ name: 'Very satisfied', color: '#4CAF50' },
				{ name: 'Satisfied', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Not satisfied', color: '#F44336' }
			]
		},
		{
			question: 'Question Restaurants 2',
			answers: [
				{ name: 'Yes', color: '#4CAF50' },
				{ name: 'Partially', color: '#FFEB3B' },
				{ name: 'No', color: '#F44336' }
			]
		}
	],
	music: [
		{
			question: 'Question Music 1',
			answers: [
				{ name: 'Very comfortable', color: '#4CAF50' },
				{ name: 'Comfortable', color: '#FFEB3B' },
				{ name: 'Acceptable', color: '#FF9800' },
				{ name: 'Uncomfortable', color: '#F44336' }
			]
		},
		{
			question: 'Question Music 2',
			answers: [
				{ name: 'Excellent', color: '#4CAF50' },
				{ name: 'Good', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Poor', color: '#F44336' }
			]
		}
	],
	'general-impressions': [
		{
			question: 'Question Overall Impression of the Mall 1',
			answers: [
				{ name: 'Excellent', color: '#4CAF50' },
				{ name: 'Good', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Poor', color: '#F44336' }
			]
		},
		{
			question: 'Question Overall Impression of the Mall 2',
			answers: [
				{ name: 'Yes', color: '#4CAF50' },
				{ name: 'Partially', color: '#FFEB3B' },
				{ name: 'No', color: '#F44336' }
			]
		}
	],
	parking: [
		{
			question: 'Question Parking 1',
			answers: [
				{ name: 'Very convenient', color: '#4CAF50' },
				{ name: 'Convenient', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Inconvenient', color: '#F44336' }
			]
		},
		{
			question: 'Question Parking 2',
			answers: [
				{ name: 'Yes', color: '#4CAF50' },
				{ name: 'Partially', color: '#FFEB3B' },
				{ name: 'No', color: '#F44336' }
			]
		}
	],
	temperature: [
		{
			question: 'Question Temperature',
			answers: [
				{ name: 'Very comfortable', color: '#4CAF50' },
				{ name: 'Comfortable', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Uncomfortable', color: '#F44336' }
			]
		}
	],
	navigation: [
		{
			question: 'Question Navigation',
			answers: [
				{ name: 'Excellent', color: '#4CAF50' },
				{ name: 'Good', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Poor', color: '#F44336' }
			]
		}
	],
	'recreation-areas': [
		{
			question: 'Question Rest Areas',
			answers: [
				{ name: 'Yes', color: '#4CAF50' },
				{ name: 'Partially', color: '#FFEB3B' },
				{ name: 'No', color: '#F44336' }
			]
		}
	],
	safety: [
		{
			question: 'Question Safety',
			answers: [
				{ name: 'Very safe', color: '#4CAF50' },
				{ name: 'Safe', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Unsafe', color: '#F44336' }
			]
		}
	],
	'mobile-app': [
		{
			question: 'Question Website',
			answers: [
				{ name: 'Very convenient Website', color: '#4CAF50' },
				{ name: 'Convenient Website', color: '#FFEB3B' },
				{ name: 'Satisfactory Website', color: '#FF9800' },
				{ name: 'Inconvenient Website', color: '#F44336' }
			]
		}
	],
	promotions: [
		{
			question: 'Question Promotions and events 1',
			answers: [
				{ name: 'Very interesting', color: '#4CAF50' },
				{ name: 'Interesting', color: '#FFEB3B' },
				{ name: 'Satisfactory', color: '#FF9800' },
				{ name: 'Not interesting', color: '#F44336' }
			]
		},
		{
			question: 'Question Promotions and events 2',
			answers: [
				{ name: 'Yes', color: '#4CAF50' },
				{ name: 'Partially', color: '#FFEB3B' },
				{ name: 'No', color: '#F44336' }
			]
		}
	],
	accessibility: [
		{
			question: 'Question Accessibility for all categories of visitors 1',
			answers: [
				{ name: 'Yes', color: '#4CAF50' },
				{ name: 'Partially', color: '#FFEB3B' },
				{ name: 'No', color: '#F44336' }
			]
		},
		{
			question: 'Question Accessibility for all categories of visitors 2',
			answers: [
				{ name: 'Yes', color: '#4CAF50' },
				{ name: 'Partially', color: '#FFEB3B' },
				{ name: 'No', color: '#F44336' }
			]
		}
	]
}
